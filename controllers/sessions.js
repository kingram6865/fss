...
async function editSession (req, res) {
  const settings = Object.keys(req.body).filter(y => {
    if (y !== 'objid') {
      return y
    }
  }).map(x => {
  ...
  try {
    const SQL = `UPDATE session_schedule SET ${settings} WHERE objid = ${req.body.objid} RETURNING *`
    console.log(SQL)
    const rows = await db.any(SQL)
    // res.json({sql: SQL})
    res.json(rows)
...
async function deleteSession (req, res) {
  try {
    const SQL = `DELETE FROM session_schedule WHERE objid = ${req.params.id}`
    const rows = await db.any(SQL)
    res.json(rows)
  } catch (error) {
    res.status(500).json(error.message)
